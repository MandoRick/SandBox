...
void countRotation() {
  digitalWrite(FLAG_PIN_OUT, HIGH);  //signal variables are being written to
  ...
  Serial.println("Rotation triggered and counted");
  Serial.println("count: " + (String)rotationCount);
  Serial.println("time: " + (String)rotationTime);
  Serial.println("rpm: " + (String)revsPerMin);
  Serial.println("speed: " + (String)finalSpeedKph);
  Serial.println("distance: " + (String)distanceTraveled);


  // this function may contains code that
  // requires heavy computation, or takes
  // a long time to execute
  digitalWrite(FLAG_PIN_OUT, LOW);  //signal variables are being written to
}
...
/*
  void renderTft() {
  if (speedKphCurrent >= 999) {
    speedKphCurrent = 0;
  }
  if (speedKphCurrent != speedKphOld) {
    tft.setTextSize(6);
    if (speedKphCurrent >= 0 && speedKphCurrent <= 9) {
      speedDigitSpacerCurrent = 105;
    } else if (speedKphCurrent >= 9 && speedKphCurrent <= 99) {
      speedDigitSpacerCurrent = 85;
    } else if (speedKphCurrent >= 99 && speedKphCurrent <= 999) {
      speedDigitSpacerCurrent = 70;
    }
    tft.setTextColor(GC9A01A_BLACK);
    tft.setCursor(speedDigitSpacerOld, 100);
    tft.println(speedKphOld);
    tft.setTextColor(GC9A01A_WHITE);
    tft.setCursor(speedDigitSpacerCurrent, 100);
    tft.println(speedKphCurrent);
  }
  speedDigitSpacerOld = speedDigitSpacerCurrent;
  speedKphOld = speedKphCurrent;
